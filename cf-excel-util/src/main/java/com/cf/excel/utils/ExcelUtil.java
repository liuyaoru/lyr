...
public class ExcelUtil {
    ...
    public static <T extends BaseRowModel> void writeExcel(HttpServletResponse response, List<T> list,
        ...
        if (excelTypeEnum == ExcelTypeEnum.XLSX) {
            ExcelWriter writer = EasyExcelFactory.getWriterWithTempAndHandler(null,
                    getOutputStream(fileName, response, excelTypeEnum), excelTypeEnum, true, new com.cf.excel.utils.WriterHandler07<>(classType));
            ...
            com.cf.excel.utils.ExcelWriterFactory writer = new com.cf.excel.utils.ExcelWriterFactory(getOutputStream(fileName, response, excelTypeEnum), excelTypeEnum);
            Sheet sheet = new Sheet(1, 0, classType);
            sheet.setSheetName(sheetName);
            try {
                writer.write(list, sheet);
     ...
     */
    public static com.cf.excel.utils.ExcelWriterFactory writeExcelWithSheets(HttpServletResponse response, List<? extends BaseRowModel> list,
    ...
    public static <T extends BaseRowModel> List<T> getExtendsBeanList(List<?> list, Class<T> typeClazz) {
        return com.cf.excel.utils.MyBeanCopy.convert(list, typeClazz);
    }
}
